        ...
        index_t vertex_count)
{
	...
	long int temp_in_degree = g_r->beg_pos[j+1] - g->beg_pos[j];
	long int degree = temp_out_degree + temp_in_degree;
...
__global__ void bfs_top_down_first(
        index_t *d_adj_list, 
        index_t *d_beg_pos, 
...
void all_wcc(index_t *root, index_t *d_root, bool *change,bool *d_change,
        index_t *d_adj_list_direct,
        index_t *d_beg_pos_direct,
        index_t *d_adj_list_reverse,
        index_t *d_beg_pos_reverse,
        index_t *vertex_status,
        index_t *d_vertex_status,
	index_t vertex_count,
	index_t *d_vertex_count,
	index_t *level,
        index_t *d_level,
...
cudaMemcpy( d_wcc_s, wcc_s, sizeof(index_t)*vertex_count, cudaMemcpyHostToDevice);
        while(*change)
        {
                (*level) ++;
                *change = false;
                ...
                {	
			//cout << root <<endl;
                        bfs_top_down_first<<<STAND_BLOCKS, STAND_THREADS>>>(
                        d_adj_list_direct,d_beg_pos_direct,d_adj_list_reverse,d_beg_pos_reverse,
                        d_vertex_status,d_change,d_vertex_count,d_level,d_wcc_s,d_root);
                }
                else{	
			//cout << root << endl;
                        bfs_bottom_up_first<<<STAND_BLOCKS, STAND_THREADS>>>(
                        d_vertex_status,d_adj_list_direct,d_beg_pos_direct,d_adj_list_reverse,
                        d_beg_pos_reverse,d_change,d_vertex_count,d_level,d_wcc_s,d_root);
                }
                cudaDeviceSynchronize();
        }
        cudaMemcpy(vertex_status, d_vertex_status, sizeof(index_t)*vertex_count, cudaMemcpyDeviceToHost);
        cudaMemcpy(wcc_s, d_wcc_s, sizeof(index_t)*vertex_count, cudaMemcpyDeviceToHost);
    //    cout << "wcc found" << '\n' << endl;
	for(index_t i = 0; i< vertex_count; i++)
        {
         if( wcc_s[i] == 0){
             *root = i;
        all_wcc(root,d_root,change,d_change,d_adj_list_direct,d_beg_pos_direct,
        d_adj_list_reverse,d_beg_pos_reverse,vertex_status,
        d_vertex_status,vertex_count,d_vertex_count,
	level,d_level,wcc_s,d_wcc_s); 
			    }
	}
}
...
int main(int args, char **argv)
{
    ...
    bit_type *wcc_s = (bit_type *)malloc(sizeof(bit_type)*vertex_count); 
    index_t *frontier_queue = (index_t*)malloc(sizeof(index_t)*vertex_count);
    
    //-------------------------pivot selection-----------------------------------

//    int root = 2;
    //index_t root = pivot_selection_first(g,g_r,vertex_count);	   
...
//    printf("blocks = %d, threads = %d\n", number_of_blocks, number_of_threads_per_block);
    ...
    bit_type *d_wcc_s;
    cudaMalloc((void**) &d_wcc_s, sizeof(bit_type)*vertex_count);
    cudaMemcpy( d_wcc_s, wcc_s, sizeof(index_t)*vertex_count, cudaMemcpyHostToDevice);
    ...
    *level = 0;
    ...
    index_t * root;
    index_t * d_root;
    cudaHostAlloc((void **) &root, sizeof(index_t), cudaHostAllocMapped);
    cudaHostGetDevicePointer(&d_root, root, 0);
    *root = 1;
    ...
    *change = true;   
    ...
    fclose(fp_time);
       for(index_t i=0; i<vertex_count; i++)
    {
        cout << vertex_status[i] << endl;
    }
    */
	
    int z_i_w = 0;
    for(index_t i=0; i<(vertex_count-1); i++)
    {
